  ...
  public List<Descriptor> findDescriptors() {
    if (descriptors == null) {
      ...
      for (octave = 0; octave < SURFMethodUtils.MAX_OCTAVE; octave++) {
        ...
        sample = (int)Math.pow(SURFMethodUtils.SAMPLING, octave); // Sample step
          ...
          for (y = 1; y < h - 1; y++) {
            for (x = 1 ; x < w - 1; x++) {
              if (KeyPoint.isMaximum(hessian, x, y, interval, threshold))
              {
                ...
                x_ = x*sample;
                y_ = y*sample;
                s_ = 0.4*((double)pow*((double)interval + 1.0) + 2.0); // box size or scale
                ...
                if(KeyPoint.interpolationScaleSpace(hessian, x, y, interval, coord, sample, pow)) {
                  ...
                  KeyPoint.addKeyPoint(intSeq, x_, y_, laplacianSign.get(interval).getValue(x, + y) == 1.0, s_, descriptors);
                }
              }
            }
          }
